...
class GalleryEstate
{
    ...
    public function save($post_id)
    {
        foreach ($this->images as $image) {
            ...
            if (array_key_exists('extension', $pathInfo)) {
                ...
                $ext = GalleryEstate::mime2ext($mime);
            }

            $path = parse_url($image['url'], PHP_URL_PATH);

            $name = str_replace('/', '-', substr($path, 1));

            $filename = wp_upload_dir()['path'].'/'.$name.'.'.$ext;
    ...
    public function getImage($url)
    {
        ...
        while ($img === null && $i < 50) {
            try {
                $img = CrawlHelper::sendGetRequest($url, CrawlHelper::getProxyService(false));
            } catch (Exception $e) {
                $img = null;
            }
            $i++;
     ...
     * @return bool
     */
    public static function imageExist($url)
    {
        global $wpdb;

        $path = parse_url($url, PHP_URL_PATH);

        $filename = str_replace('/', '-', substr($path, 1));
